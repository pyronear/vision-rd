"""PyTorch Lightning wrapper around :class:`TubeMultiscaleClassifier`."""
...
class LitTubeMultiscaleClassifier(L.LightningModule):
    """BCE loss; AdamW with two param groups when finetuning the backbone.

    Logs train/val loss, accuracy, precision, recall, F1.
    """
    ...
    def __init__(
        ...
        self.model = TubeMultiscaleClassifier(
            backbone=backbone,
            finetune=finetune,
    ...
    def _trainable_non_backbone_params(self) -> list[torch.nn.Parameter]:
        backbone_param_ids = {
            id(p) for p in self.model.global_branch.backbone.parameters()
        }
        return [
            p
    ...
    def configure_optimizers(self):
        non_backbone = self._trainable_non_backbone_params()
