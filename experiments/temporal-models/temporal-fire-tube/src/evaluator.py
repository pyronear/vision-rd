...
def load_prediction_results(results_path: Path) -> list[dict]:
...
def compute_metrics(results: list[dict]) -> dict:
    ...
    Returns dict with: num_sequences, tp, fp, fn, tn, precision, recall,
    f1, fpr, mean_ttd_seconds, median_ttd_seconds.
    """
...
def plot_confusion_matrix_percentages(metrics: dict, output_path: Path) -> None:
    """Plot a confusion matrix heatmap with percentages."""
    sns.set_theme(style="whitegrid")
    cm = [[metrics["tp"], metrics["fn"]], [metrics["fp"], metrics["tn"]]]
...
def plot_comparison(
    yolo_metrics: dict, fire_tube_metrics: dict, output_path: Path
) -> None:
    """Bar chart comparing YOLO-only vs fire-tube on precision, recall, and F1."""
    sns.set_theme(style="whitegrid")
    data = pd.DataFrame(
        {
            ...
            "Method": ["YOLO-only"] * 3 + ["Fire-Tube"] * 3,
        }
    )
    fig, ax = plt.subplots(figsize=(8, 5))
    sns.barplot(data=data, x="Metric", y="Score", hue="Method", ax=ax)
    ax.set_ylim(0, 1.05)
    ax.set_title("YOLO-only vs Fire-Tube")
