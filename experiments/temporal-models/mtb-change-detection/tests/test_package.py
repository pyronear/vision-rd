...
from mtb_change_detection.package import (
    CONFIG_FILENAME,
    FORMAT_VERSION,
    MANIFEST_FILENAME,
    WEIGHTS_FILENAME,
...
class TestConfigSchema:
    ...
    def test_change_keys(self, built_archive: Path) -> None:
        with zipfile.ZipFile(built_archive, "r") as zf:
            config = yaml.safe_load(zf.read(CONFIG_FILENAME))
        assert set(config["change"]) == {
            "pixel_threshold",
            "min_change_ratio",
        }
...
class TestRoundtrip:
    @patch("mtb_change_detection.package.load_model")
    def test_config_values_match(
        ...
        assert pkg.change_params["pixel_threshold"] == 10
        assert pkg.change_params["min_change_ratio"] == 0.01
        assert pkg.prefilter_params["confidence_threshold"] == 0.01
        assert pkg.prefilter_params["max_detection_area"] == 1.0
        assert pkg.tracker_params["iou_threshold"] == 0.1
        assert pkg.tracker_params["min_consecutive"] == 2
...
class TestCreateTracker:
    ...
    def test_tracker_params_match(
        ...
        assert tracker.min_consecutive == 2
