...
def plot_comparison(
    ...
    """Bar chart comparing YOLO-only vs MTB+Tracking on precision, recall, F1.

    Args:
        yolo_metrics: Metrics dict for the YOLO-only baseline.
        tracking_metrics: Metrics dict for the MTB+Tracking model.
        output_path: Destination file path for the PNG image.
    """
    sns.set_theme(style="whitegrid")
    data = pd.DataFrame(
        {
            ...
            "Method": ["YOLO-only"] * 3 + ["MTB+Tracking"] * 3,
        }
    )
    fig, ax = plt.subplots(figsize=(8, 5))
    sns.barplot(data=data, x="Metric", y="Score", hue="Method", ax=ax)
    ax.set_ylim(0, 1.05)
    ax.set_title("YOLO-only vs MTB+Tracking")
