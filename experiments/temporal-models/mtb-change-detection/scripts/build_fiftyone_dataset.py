"""Build a FiftyOne dataset from MTB tracking pipeline outputs.
...
from mtb_change_detection.data import get_sorted_frames, load_label_boxes
from mtb_change_detection.detector import load_inference_results
from mtb_change_detection.evaluator import load_tracking_results
from mtb_change_detection.types import Detection
...
def build_dataset(
    ...
    for result in tqdm(results, desc=f"Building {split} dataset"):
        ...
        # Get sorted image paths (nested layout)
        seq_dir = _find_sequence_dir(data_dir, seq_id)
        if seq_dir is None:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Build a FiftyOne dataset from MTB tracking outputs."
    )
    parser.add_argument(
        "--split",
        type=str,
    ...
    dataset_name = args.dataset_name or f"mtb-change-detection-{args.split}"
    build_dataset(
        split=args.split,
