"""PyTorch Dataset for the tube multiscale fusion classifier.

Reads cropped PNG patches produced by ``scripts/build_model_input.py`` and
returns per-tube tensors padded to a fixed length with a boolean mask.
"""
...
class TubePatchDataset(Dataset):
    ...
    When ``transform`` is ``None``, patches are ImageNet-normalized in place.
    Otherwise patches reach ``transform`` as un-normalized ``[0, 1]`` tensors
    and the transform is responsible for normalization as its final step.
    ...
    def __init__(
        ...
        self.index: list[dict] = json.loads(
            (self.split_dir / "_index.json").read_text()
        )
