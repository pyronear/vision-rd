"""Bundle YOLO weights and tracking config into a single model archive.

Reads the experiment ``params.yaml``, extracts the inference and tracking
parameters, and packages them together with the YOLO weights into a ``.zip``
...
from tracking_fsm_baseline.package import build_model_package
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Bundle YOLO weights and tracking config into a model archive."
    )
    parser.add_argument(
        "--weights-path",
        type=Path,
