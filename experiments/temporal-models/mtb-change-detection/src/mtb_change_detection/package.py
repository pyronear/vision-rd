"""Model packaging: bundle YOLO weights and MTB+tracking config into a single archive.
...
- ``config.yaml`` -- Inference, change detection, prefilter, and tracker parameters.

This allows shipping the complete smoke-detection pipeline (detector + MTB
change validation + FSM tracker settings) as one self-contained artefact.
"""
...
class ModelPackage:
    ...
    @property
    def change_params(self) -> dict[str, Any]:
        """MTB change detection parameters."""
        return self.config["change"]

    @property
    def prefilter_params(self) -> dict[str, Any]:
        """Detection pre-filter parameters (applied before change validation)."""
...
def build_model_package(
    ...
    """Bundle YOLO weights and MTB+tracking config into a ``.zip`` archive.

    Args:
        weights_path: Path to the YOLO ``.pt`` weights file.
        params: Full ``params.yaml`` dictionary (must contain ``infer``,
            ``pad``, ``change``, and ``track`` sections).
...
def _build_config(params: dict[str, Any]) -> dict[str, Any]:
    ...
    into the package config schema with ``infer``, ``pad``, ``change``,
    ``prefilter``, and ``tracker`` sections.
    """
    infer = params["infer"]
    pad = params["pad"]
    ...
    return {
        ...
        "change": {
            "pixel_threshold": change["pixel_threshold"],
            "min_change_ratio": change["min_change_ratio"],
        },
        "prefilter": {
