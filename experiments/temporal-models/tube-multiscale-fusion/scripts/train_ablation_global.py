"""Train a global-branch ablation variant (no_spatial or weighted_mean).

Reads the variant's training block plus the shared ``global_branch`` /
``local_branch`` / ``fusion`` sections so the retained components match the
full model byte-for-byte.
"""
